...
function CoinRequests() {
  ...
  const getAllRequests = async () => {
    setLoading(true);
    try {
      const response = await Get(COIN.coinRequests, token, {...filter, page: page?.toString(), limit: limit?.toString()});
      setLoading(false);
    ...
    <Layout className="configuration">
      <div className="boxDetails">
        <Row style={{ padding: "10px 20px" }}>
          ...
          >
            <h1 className="pageTitle">Red Coins Requests</h1>
          </Col>
        </Row>
        {/* <Row style={{ padding: "10px 20px" }}>
