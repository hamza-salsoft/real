...
function IncommingGoldCoins() {
  ...
  console.log("🚀 ~ IncommingGoldCoins ~ paginationConfig:", paginationConfig)
  const { limit, pageNumber: page } = paginationConfig;
  console.log("🚀 ~ IncommingGoldCoins ~ page:", page)
  ...
  const getAllRequests = async () => {
    setLoading(true);
    try {
      const response = await Get(COIN.redeemRequests, token, {...filter, page: page?.toString(), limit: limit?.toString()});
      setLoading(false);
    ...
    <Layout className="configuration">
      <div className="boxDetails">
        <Row style={{ padding: "10px 20px" }}>
          ...
          >
            <h1 className="pageTitle">Incomming Gold Coins</h1>
          </Col>
        </Row>
        {/* <Row style={{ padding: "10px 20px" }}>
...
export default IncommingGoldCoins;
