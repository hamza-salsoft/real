...
import { BOOK, QUESTION } from "../../config/constants";
import { useSelector } from "react-redux";
import { useNavigate } from "react-router-dom";
import { ImageUrl } from "../../config/functions";

function SignupFormManagement() {
  ...
  const resetFilter = () => {
    ...
    getBooks(paginationConfig.pageNumber, paginationConfig.limit, "", true);
  };
  ...
  const getBooks = async (pageNumber, pageSize, search, reset = false) => {
    setLoading(true);
    try {
      const response = await Get(QUESTION.getAllQuestions, token, {
        page: pageNumber
          ? pageNumber.toString()
    ...
    // },
    {
      ...
      render: (item) => (
        <FaEye
          ...
          onClick={() => navigate("/signUpForms/editQuestion/" + item)}
        />
      ),
    },
  ];
    ...
    <Layout className="configuration">
      <div className="boxDetails">
        <Row style={{ padding: "10px 20px" }}>
          ...
          >
            <h1 className="pageTitle">Signup Form Management</h1>
          </Col>
          <Col
            xs={24}
            md={12}
          ...
          >
            <Button
              ...
              className="mainButton primaryButton"
              onClick={() => navigate("/signUpForms/addQuestion")}
            >
              Add Question
            </Button>
          </Col>
            ...
            <Input
              ...
              suffix={
                <FaSearch
                  ...
                  onClick={() =>
                    getBooks(1, paginationConfig.limit, filter.keyword)
                  }
                />
              }
              onPressEnter={(e) =>
      ...
      >
        ...
        <Typography.Title level={4} style={{ fontSize: "25px" }}>
          {selectedBook?.status == "ACTIVE" ? "Deactivate" : "Activate"}
        </Typography.Title>
        <Typography.Text style={{ fontSize: 16 }}>
          Do You Want To{" "}
          {selectedBook?.status == "ACTIVE" ? "Deactivate" : "Activate"} This
          Book?
        </Typography.Text>
      </Modal>
    </Layout>
...
export default SignupFormManagement;
